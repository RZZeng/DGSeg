...
class Sam3Image(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self._init_pixel_prompt_fusion(d_model=self.hidden_dim, pixel_dim=256, num_heads=8, dropout=0.1, ffn_dim=512)

    def _init_pixel_prompt_fusion(
        self,
        d_model: int,
        pixel_dim: int = 256,
        num_heads: int = 8,
        dropout: float = 0.1,
        ffn_dim: int = 512,          # 不要太大，避免过拟合
        use_proj_if_mismatch: bool = True,
    ):
        """
        pixel_embed (B, pixel_dim, H, W) -> tokens (HW, B, d_model)
        prompt     (L, B, d_model)
        cross-attn: q <- attend(prompt)
        output: logits (B,1,H,W)
        """
        self.fuse_dropout = dropout
        self.fuse_num_heads = num_heads
        self.fuse_ffn_dim = ffn_dim
        self.fuse_pixel_dim = pixel_dim
        self.fuse_use_proj_if_mismatch = use_proj_if_mismatch

        # 若 pixel_dim != d_model，用 1x1 conv 投影到 d_model
        self.pixel_q_proj = None
        if use_proj_if_mismatch and pixel_dim != d_model:
            self.pixel_q_proj = nn.Conv2d(pixel_dim, d_model, kernel_size=1)

        # prompt dim 理论上就是 d_model；如果你担心不一致，也可加线性投影
        self.prompt_kv_proj = None  # 一般不需要

        # 一个轻量 cross-attn（相当于 decoder 的 cross_attn）
        ...
        # pre-norm 更稳
        self.fuse_q_norm = nn.LayerNorm(d_model)
        self.fuse_kv_norm = nn.LayerNorm(d_model)
        ...
        self.fuse_ffn_norm = nn.LayerNorm(d_model)

        # 输出 logits：你提的 256->1 conv（这里用 d_model -> 1）
        # 为了抗过拟合，前面加一个轻量 dropout2d（不加也行）
        self.fuse_logits_head = nn.Sequential(
    ...
    def forward_fusion_from_pixel_embed(
        self,
        backbone_out,
        find_input,
        query,
        ...
        text_outputs = self.backbone.forward_text([query], device=self.device)
        geometric_prompt = self._get_dummy_prompt()
        backbone_out.update(text_outputs)
        # branch 1: only-text branch
        with torch.profiler.record_function("SAM3Image._encode_prompt"):
            prompt, prompt_mask, backbone_out = self._encode_prompt(
                backbone_out, find_input, geometric_prompt
            )

        pixel_embed = self.segmentation_head.embed_pixels(
                backbone_feats=backbone_out["backbone_fpn"],
        ...
        q = pixel_embed.flatten(2).permute(2, 0, 1)
        q = self.fuse_q_norm(q)


        with torch.profiler.record_function("fusion_cross_attn"):
            attn_out, _ = self.fuse_cross_attn(
                query=q,
                key=prompt,
                value=prompt,
                key_padding_mask=prompt_mask,  # (B, L), True=pad(一般是这样)
                need_weights=False,
            )
            q = q + F.dropout(attn_out, p=self.fuse_dropout, training=self.training)

        with torch.profiler.record_function("fusion_ffn"):
            q2 = self.fuse_ffn_norm(q)
            q = q + self.fuse_ffn(q2)

        # reshape 回图
        fused_feat = q.permute(1, 2, 0).contiguous().view(B, C, H, W)

        logits = self.fuse_logits_head(fused_feat)  # (B, 1, H, W)

        return {
            "fusion_logits": logits,
            "fusion_pixel_feat": fused_feat,   # 可选：便于 debug
            "pixel_embed_raw": pixel_embed,    # 可选：便于对比
        }
