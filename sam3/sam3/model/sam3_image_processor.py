...
class Sam3Processor:
    ...
    @torch.inference_mode()
    def set_text_prompt_alloutput(self, prompt: str, state: Dict):
        """Sets the text prompt and run the inference"""
        ...
        return self._forward_grounding_alloutput(state)

    @torch.inference_mode()
    def add_geometric_prompt_alloutput(self, box: List, label: bool, state: Dict):
        """Adds a box prompt and run the inference.
        The image needs to be set, but not necessarily the text prompt.
        The box is assumed to be in [center_x, center_y, width, height] format and normalized in [0, 1] range.
        The label is True for a positive box, False for a negative box.
    ...
    @torch.inference_mode()
    def _forward_grounding_alloutput(self, state: Dict):
        outputs = self.model.forward_grounding(
            backbone_out=state["backbone_out"],
            find_input=self.find_stage,
            geometric_prompt=state["geometric_prompt"],
        ...
        outputs["backbone_out"] = state["backbone_out"]
        outputs["masks_logits"] = out_masks
        outputs["masks"] = out_masks > 0.5
        outputs["boxes"] = boxes
        outputs["scores"] = out_probs
        return outputs