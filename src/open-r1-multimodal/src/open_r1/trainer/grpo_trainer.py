...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # ZeRO-3 + GRPO's multi-forward workflow is fragile when generation temporarily
        # gathers params and removes/re-adds hooks. Running generation on sharded params
        # is slower, but it avoids the DeepSpeed trace corruption seen in full FT.
